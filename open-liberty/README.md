...
[![mips64le/open-liberty build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/mips64le/job/open-liberty.svg?label=mips64le/open-liberty%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/mips64le/job/open-liberty/)
...
```dockerfile
FROM mips64le/open-liberty:kernel
...
2.	The `full` images provide the library cache directory, `lib.index.cache`, which contains an indexed library cache created by the `springBootUtility` command. Use `lib.index.cache` to provide the library cache for a thin application.
	...
	```dockerfile
	FROM mips64le/open-liberty:kernel as staging
...
docker run -v /opt/ol/wlp//output/.classCache \
    --name classcache mips64le/open-liberty true
```
...
docker run -d -p 80:9080 -p 443:9443 \
    ...
    mips64le/open-liberty:webProfile8
```
