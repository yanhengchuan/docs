...
```console
$ docker run -it --rm --name my-running-script -v "$PWD":/usr/src/myapp -w /usr/src/myapp mips64le/perl:5.20 perl your-daemon-or-script.pl
```
...
```console
$ docker run -it --name sleeping_beauty --rm mips64le/perl:5.20 perl -E 'sleep 300'
```
...
```console
$ docker run -it --name quick_nap --rm mips64le/perl:5.20 perl -E '$SIG{TERM} = sub { $sig++; say "recv TERM" }; sleep 300; say "waking up" if $sig'
```
