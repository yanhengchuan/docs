...
```console
$ docker run --rm mips64le/influxdb influxd config > influxdb.conf
```
...
$ docker run -p 8086:8086 \
      ...
      mips64le/influxdb -config /etc/influxdb/influxdb.conf
```
...
```console
$ docker run --name=influxdb -d -p 8086:8086 mips64le/influxdb
```
...
```console
$ docker run --rm --link=influxdb -it mips64le/influxdb influx -host influxdb
```
